...
- The 400 ms render debounce is a coalescing delay, not a concurrency lock. A lower-power replacement should use a single-flight render gate (`renderBusy`) plus dirty/priority flags, clearing the gate in `finally` and servicing one queued render afterward; a lock alone would drop updates or cause redundant retries.
